    ...
    return (
      <div className="mt-6">
        ...
        <div className="max-h-64 overflow-y-auto space-y-3">
          {[...Array(5)].map((_, index) => (
            <div key={index} className="animate-pulse flex justify-between items-start text-sm border-b border-stone-100 pb-2">
      ...
      <div className="max-h-64 overflow-y-auto space-y-3">
        {transactions.length === 0 ? (
          ...
          transactions.map((transaction) => (
            <div
              key={transaction.id}
              className="flex justify-between items-start text-sm border-b border-stone-100 pb-2 last:border-b-0"
            >
              <div className="flex-1">
                ...
                <div className="mt-1 space-y-1">
                  ...
                  {/* User Info */}
                  <div className="flex items-center space-x-1 text-xs text-stone-700">
                  ...
                  {transaction.admin_email && (
                    <div className="flex items-center space-x-1 text-xs text-brand-600">
                      <FiUser className="w-3 h-3" />
                      <span title={`Adjusted by ${transaction.admin_email}`} className="text-brand-600">
